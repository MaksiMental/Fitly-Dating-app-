  ...
  <div>
    <div class="container">
      ...
      <form @submit.prevent="onSubmit">
        ...
        <div class="form-group">
          <button class="btn btn-primary btn-block btn-lg">Ændre indstillinger for denne brugers id</button>
        </div>
        <div>Tilbage til Admin Panel? <router-link to="/adminpanel">Panelet</router-link></div>
      </form>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    onSubmit() {
      var token = localStorage.getItem("xauthtoken"); // Gets the JWT token from localStorage (Set when logged in or registered)
      axios
        .put("http://127.0.0.1:7071/api/authorization_users", {
          id: this.id,
          email: this.email,
        ...
        .then((response) => {
          ...
          if (response.status == 200) {
            alert("Profilen er blevet opdateret")
          }
